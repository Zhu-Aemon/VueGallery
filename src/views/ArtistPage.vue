<template>
	...
	<div class="ml-20 text-xl font-medium">
	      ...
	      ? 'cursor-pointer bg-gray-200 rounded-2xl py-2 px-4 text-blue-700 font-bold'
	            ...
	            @click="setHottest">最热单曲</span>
		<span :class="showHottest === false
...
onMounted(async () => {
	...
	await axios.get(`http://localhost:3000/artist/top/song?id=${artistId.value}`).then((response) => {
...
const setAll = async () => {
	...
	await axios.get(`http://localhost:3000/artist/songs?id=${artistId.value}&limit=200`).then((response) => {
		artistSongs.value = response.data.songs
